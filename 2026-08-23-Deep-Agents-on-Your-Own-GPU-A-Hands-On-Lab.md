...
tags:
  ...
  - python
header:
  teaser: /assets/images/posts/2026-08-23-Deep-Agents-Ollama/deep-agents-ollama.jpg
toc: true
toc_sticky: true
---
...
Almost everything that goes wrong here is one of six things, and none of them announce themselves.

**The agent explains what it *would* do and never calls a tool.** The model can't do tool calls. Switch to `qwen3:8b` and confirm with check 6 of `make doctor`.

**It forgets the task halfway through.** Your context window is too small and Ollama truncated it without saying so. Raise `DEEP_AGENT_NUM_CTX`; checks 4 and 5 tell you what fits.

**Answers get worse the longer it runs.** The `profile` is missing, so compaction never fires. Add `profile={"max_input_tokens": num_ctx}` — section 12.

**Every step pauses for a few seconds.** Ollama is unloading the model between calls. Set `keep_alive`, which `.env.example` already does. If delegating to a sub-agent is *especially* slow, you have two models fighting over VRAM — use one for everyone.

**Everything crawls and the GPU looks idle.** You've spilled into system RAM. Lower `num_ctx`, or set `OLLAMA_KV_CACHE_TYPE=q8_0` to roughly halve the KV cache.

**`cannot reach Ollama`.** The daemon isn't running: `ollama serve`, or start the desktop app.

**It printed the banner and then nothing.** Almost always it is working, not stuck — a deep-agent run on a local 8B takes minutes, and the labs stream a line per model and tool call so you can see it moving. If the lines have genuinely stopped, `make doctor` now measures this for you: check 6b loads the model and reports how much of it landed on the GPU. Anything below 100% means the rest is running on the CPU and the whole run goes at that speed. Lower `DEEP_AGENT_NUM_CTX` or pick a smaller model.

**`TAVILY_API_KEY is not set`, or a 403 exporting traces.** You ran `make env` and never edited `.env`, so the placeholders are still in it. The labs check for this before the first model call now — a placeholder counts as missing, which is the whole point, since `pk-lf-...` is not empty and an emptiness test waves it through.

Two error messages worth recognising on sight. `TypeError: unhashable type: 'slice'` means you sliced a `FileData` dict — use `data["content"]`. `write_todos not found` means you forgot `middleware=[TodoListMiddleware()]`.
...
Then we gave it a desk — a plan, some files, and helpers with their own fresh conversations. The same task that had confused the plain loop finished in eighteen clear messages. And we watched it pick its own way through the toolbox: plan, look around, narrow, read, delegate, write.
...
- Point Lab 3 at a **real project of yours** instead of the toy one. Read your files off disk into the `files` dict and ask for the audit.
- Add a **second helper** — a critic that reads `report.md` and complains about it before you see it.
- Write **middleware that retries** a failed tool call once before giving up, using `wrap_tool_call` (section 14.1).
- Swap `StateBackend` for a **persistent backend** so the agent's notes survive between runs.
- Turn on **human-in-the-loop** so you approve tool calls before they run.
- Run the same task on `qwen3:8b` and `qwen3:14b` and diff the traces. The difference in how well they stick to a plan is more interesting than any benchmark.
